...
- **topics directory** — the Topics tab's grouped view, the full content of `_topic_directory_tab.html.erb` (ADR 0013). Browse-first: students finding topics to propose to, lecturers browsing, coordinators reviewing every status via `TopicPolicy::Scope`. Stacked action bar (`+ Create` top-left, search pill, Topic filter + Collapse all), supervisor groups with collapse, per-topic `_row_item` rows (with the **available badge**). Unrelated to the standalone `topics/index` page.
- **supervisor group** — a per-supervisor collapsible block in the topics directory: **section header** (its `row_id:` mode: name + topic count + chevron) + **row list** underneath. Outer loop on the course's lecturers; zero-topic supervisors still render ("Kevin — 0"). Groups sort A→Z; rows newest-first. No capacity figures in headers. Reuses `expandable_rows_controller.js` for collapse (generalized from its Groups-table-only selectors).
